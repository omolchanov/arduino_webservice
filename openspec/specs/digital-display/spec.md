...
The Digital Signal dashboard SHALL include a resistance widget that displays the live LED resistance reading formatted to one decimal place with a **Ω** unit label. The widget SHALL be placed in the same row as the potentiometer, detected logic LED, and current widgets. The widget SHALL include a status dot that follows the same online/offline and pending-reading pattern as sensor widgets on the Sensors dashboard.
...
The Digital Signal dashboard SHALL include a resistance history graph titled **Resistance history, Ω** that plots LED resistance over time as a line chart with a rolling client-side history buffer. The Y-axis SHALL be labeled **Ω** and range from `0` to `50` Ω. The graph SHALL appear immediately after the shunt resistor history (mA) graph on the dashboard.
...
- **THEN** the Y-axis displays the unit label `Ω` and values from `0` to `50`
